...
- 上下文占用超过LLM窗口70%时才触发压缩，而非固定max_rounds=2
- 短对话可保留更多轮（充分利用窗口），长对话自动缩减（防止溢出）
- `get_conversation_history_text` 默认 max_rounds=0（动态计算）
- `update_clinical_snapshot_node` 从固定6条消息触发改为token占用率触发
- 优点：上下文利用率最优化，不会过早截断也不会过晚
- 缺点：token估算是粗略的（字符数×1.5），实际占比可能有±10%偏差；需要配置正确的LLM_CONTEXT_WINDOW_TOKENS

### P2: 压缩时原文存档（不丢弃）
- 快照提取前，将早期消息原文完整存入磁盘（data/persisted_outputs/archives/）
- clinical_checkpoint 中记录 archive_ids 列表，便于按需回溯
