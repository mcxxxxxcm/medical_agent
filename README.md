...
- **两级缓存**：L0 答案缓存（精确匹配）、L2 语义缓存（相似度匹配）
...
### 🧠 上下文管理（医疗级）
- **结构化临床快照**：对话压缩时生成 JSON 格式临床状态（主诉/症状时间线/用药记录/高危症状/过敏史/排除项），替代自由文本摘要，防止关键临床坐标丢失
- **冻结用户画像**：过敏史、既往病史等长期属性独立注入 System Prompt，永不参与压缩，杜绝"摘要的摘要"丢失过敏信息
- **RAG 文档剥离**：历史对话中的检索文档自动替换为引用标记（MicroCompact），防止上下文被旧医学文献塞满
...
│  │ (Checkpoints│  │ (L0+L2 Cache)│  │   (Vector Store)    │  │
│  └─────────────┘  └─────────────┘  └─────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
...
- [x] 结构化临床快照（替代自由文本摘要）
- [x] 冻结用户画像（过敏史永不丢失）
- [x] RAG文档剥离（MicroCompact）
- [x] MinerU PDF解析（替代PyPDF/Docling）
- [ ] 图片识别功能（主服务未开放）
- [ ] RAGAS 自动评估
- [ ] 并行检索架构
- [ ] 全节点异步化
