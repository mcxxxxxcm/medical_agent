...
## v8.4 - TTFT 优化：症状解析规则增强 + HyDE 短路修复（预期 6.3s → 2.7s）

### 背景

分析"流鼻血了怎么办？"请求日志，TTFT = 6269ms，耗时分布：

| 节点 | 耗时 | 占比 | 问题 |
|------|------|------|------|
| 症状解析 | 2809ms | 44.8% | 规则没命中"流鼻血" → LLM 白跑 → 结果校验失败 → None |
| HyDE 生成 | 734ms | 11.7% | 自包含查询仍执行 HyDE |
| 知识检索 | 1559ms | 24.9% | embedding API 730ms |
| LLM 首token | 1042ms | 16.6% | 智谱 API 延迟 |

### 修复1：症状解析关键词覆盖（2809ms → 0ms）

"流鼻血/鼻出血"不在 `SYMPTOM_MAP` 中，规则提取返回 None → 走 LLM → 校验失败 → 白花 2.8s。

新增关键词：`流鼻血`、`鼻出血`、`鼻流血`、`便血`、`咯血`、`尿血`、`血尿`

### 修复2：HyDE 短路条件补全（734ms → 0ms）

```python
# 旧条件
_hyde_symptom_words = [..., "出血", ...]
# "流鼻血了怎么办？" → "出血" in "流鼻血了怎么办？" = False（"出血"不是子串）

# 新条件
_hyde_symptom_words = [..., "出血", "流血", "血", ...]
# "流鼻血了怎么办？" → "血" in "流鼻血了怎么办？" = True → skip_hyde = True
```
...
| 节点 | 修复前 | 修复后 | 节省 |
|------|--------|--------|------|
| 症状解析 | 2809ms（LLM） | ~5ms（规则） | 2804ms |
| HyDE | 734ms（LLM） | 0ms（跳过） | 734ms |
| 知识检索 | 1559ms | 1559ms | — |
...
| 文件 | 改动 |
|------|------|
| `app/core/keyword_matcher.py` | 症状映射新增"流鼻血/鼻出血/便血/咯血/尿血"等7个关键词 |
| `app/graph/nodes/nodes.py` | `_hyde_symptom_words` 新增"流血"、"血" |
