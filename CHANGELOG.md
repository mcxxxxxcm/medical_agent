...
- **新增 `--loose-threshold`（默认0.6）宽松通过率**：`evaluate_golden_match.py` 报告与 JSON 输出新增 `loose_threshold`/`loose_passed`/`loose_pass_rate`，判定"内容已答到≥0.6 要点"为宽松通过。`_print_report` 顶部与控制台同步展示「阈值(严格) 0.7 → 46.3%　宽松(参考) 0.6 → 70.4%」。
- **严格口径完全不动**：原 `--threshold` 默认仍 0.7，`passed`/`pass_rate`/`threshold` 字段及 `per_sample` 逐条判定全部保持原值，历史报告对比不回退。宽松通过仅作并行参考，不进 `passed`，不影响优化方向的严格标尺压力。
- 设计取舍：不改判定逻辑、只加"内容覆盖"视角——既认可"答到大部分要点"的真实质量，又不放松"全对才算过"的生产级严格下限。

验证（`my_medical_env`，`ENABLE_SEMANTIC_CACHE=false`，温度0）：golden 全量权威结果双口径——
- **严格 0.7：25/54 = 46.3%**（与 v9.73 持平/略升，评测温度随机性致 +1 抖动）
- **宽松 0.6：38/54 = 70.4%**——即按"答到大部分要点"口径，系统实际覆盖质量约七成，0.7 严格多判掉的 13 个样本多为只差 1 个要点。
- 诚实结论：双阈值不是"放宽门槛掩盖低质"，而是把「全对才过的生产门槛」与「内容已答到大部分要点的质量视图」并列，让 v9.73 检索增强的真实收益（拨打120 4/7、扭伤 1.0 等）在宽松口径下看得更清，也为后续优化是否该攻"补齐临近要点"提供依据。

<footer>v9.73b · scripts/evaluate_golden_match.py、CHANGELOG.md</footer>

## v9.73 - 急诊"需立即拨打120"跨文档召回增强：列举型急诊问题补回独立急救文档（app/graph/nodes/nodes.py）

背景：真·生产黄金基线 v9.70 为 42.6%，用户主攻急诊跨章节召回。关缓存诊断「什么情况需要立即拨打120？」（7要点：心梗胸痛>30分钟/脑卒中FAST/严重呼吸困难/过敏性休克/大出血/意识丧失/高热惊厥>5分钟），发现**召回19段却只命中1个心梗章节 + 一堆泛化头痛/冠心病/哮喘章节**，真正的急诊急救独立文档一个都没召回。该类查询的正确答案**分散在 4~6 个不同源文档**（心梗→`急性心肌梗死识别`、脑卒中FAST→`急性脑卒中_FAST`、过敏性休克→`过敏性休克识别`、呼吸困难/意识障碍/大出血→`急诊常见症状识别与处理`、高热惊厥→`儿童常见疾病护理`），而现有链路**无法跨文档聚合**：① 查询无具体病名实体，Dense/BM25 定位不到以病名作标题的独立文档；② 邻域扩展 `expand_with_siblings` 只在同 `doc_id` 内拉兄弟章节，跨独立文档天生拉不到；③ K=8 但 top-k 被头痛/哮喘等语义相关"列表型"章节挤占，独立急救文档排在 k 外。

- **急诊跨文档召回增强（`knowledge_retrieval_node`）**：新增 `_EMERGENCY_ENUM_TRIGGER_RE`（拨打120/叫救护车/哪些情况…急救就医）、`_EMERGENCY_SCENARIO_QUERIES`（7 个急诊场景子查询，对齐各独立急救文档标题词）、`_is_emergency_enum_query`。仅当查询为**通用急诊清单**（不含具体病名/症状实体）时，把子查询注入并行检索，复用现有 `len(sub_questions)>1` 的 ThreadPoolExecutor 分支把独立急救文档补进候选。纯检索侧改动，不改生成 prompt/final_question，不做急症断言；普通单病种问答零触发。
- **触发过宽修复（病名实体排空）**：初版正则 `(哪些|什么)情况…就医/急诊` 过宽，把单病种需就医指征（"头痛…就医""荨麻疹…急诊"）也拉进跨病种聚合，杂入脑卒中/过敏噪声反致漏答。改为用 `extract_entity_terms` 检测查询中除急诊泛词之外的**具体病名/症状实体**，有则跳过——单病种只由自身章节回答，通用清单才跨文档聚合。
- **不改生成、不做诊断**：急诊子查询只走检索路径；答案 prompt 仍用完整原问题。符合 v9.51 起"检索增强不动 final_question"与"不做诊断断言"两个既有硬约束。

验证（`my_medical_env`，`ENABLE_SEMANTIC_CACHE=false`，温度0 确定性标尺，LLM 语义判定）：
- **检索层诊断**：「什么情况需要立即拨打120？」召回从 19 段（几乎全是泛化章节）→ 覆盖全部急救独立文档（FAST/过敏性休克/心梗/意识障碍/大出血/热性惊厥章节），要点召回率 3/7（LLM 语义复审网络超时，按字面口径 3/7，检索候选已覆盖全部 7 要点）。
- **黄金评测通过率**：v9.70 基线 23/54=42.6% → 修复后 **24/54=44.4%**（avg_hit 0.678→0.653）。净 +1、**无样本由通过转失败**。
- **关键样本**：「什么情况需要立即拨打120？」0.43→0.57（提升未过0.7，语义命中心梗/过敏性休克/意识丧失，脑卒中FAST/大出血/高热惊厥仍未写全，留待下步）；「头痛…就医」0.83 恢复通过（过宽时曾掉到 0.67）；「荨麻疹…急诊」正确回落单病种 0（过宽时靠跨病种过敏文档虚高 0.5）；「扭伤冷敷热敷」0.40→1.00（顺带受益）。
- 诚实结论：整体通过率提升不大（+1），因唯一显著进步的拨打120 从 0.43 到 0.57 仍卡在 0.7 阈值下——跨文档召回已把段落喂全，但生成层把 7 要点写全还差临门一脚，且上下文过载（94 段）可能稀释。下一步若继续冲该样本，需在生成侧聚焦"把已召回的多类急诊场景逐一转述"，而非再加检索量。

<footer>v9.73 · app/graph/nodes/nodes.py、CHANGELOG.md</footer>
...
- **评测检索完整复用生产节点（evaluation.py `run_retrieval`）**：直接调生产 `knowledge_retrieval_node`，把路由（`detect_rule_based_route`→`_llm_route`）、查询预处理、剥过敏、question_type 驱动 K 选择、symptom 增强、父子/兄弟扩展(2800字符)、来源去重、`filter_relevant_docs` 全部照单复用——保证**评测 = 生产**。评测为单轮样本、无历史，故不套上下文感知路由层（该层依赖对话历史）；不跑 symptom_analysis，`symptoms` 置空（方向增强对空症状安全 no-op，故评测略欠方向增强、是"真·生产"的保守下限）。
- **修复生产端知识检索静默崩溃（nodes.py:1804）**：`_is_detail_soliciting_query(question_type, question)` 引用了未定义的 `question`（局部只定义了 `original_query`）→ `NameError`，被节点 except 吞掉后**对每条 knowledge 类查询静默返回空检索**。改为 `original_query`。即之前生产端知识问答一直返回空（答不出"什么是高血压"），评测用旧路径绕开了这个节点所以没暴露。验证：什么是高血压 0→13 段、高血压日常注意 0→19、糖尿病的症状 0→12。这是**真实生产修复**，非评测口径变化。

验证（`my_medical_env`，`ENABLE_SEMANTIC_CACHE=false`，温度0 确定性标尺）：
- **重新建标尺（对齐后真·生产通过率）：26/54=48.2% → 23/54=42.6%**，avg_hit 0.678→0.643。**下降是口径变诚实，不是回退**：旧 48.2% 建立在"半残检索"之上的虚高；真·生产经增强 + 兄弟扩展 2800 的**更全但更噪**的上下文喂给生成，稀释了精确要点措辞（剂量查询 0/4、症状分诊 33%、急救 45% 最弱）。
- 分档通过率（对齐后）：剂量查询 0/4、症状分诊 4/12、急救处理 5/11、知识问答 3/7、慢性病管理 3/6、用药安全 8/14。
- 诚实结论：① 此前 v9.67/69 记录的 46.3→48.2 提升是"半残系统"口径下的数，真·生产评测应以此为**新基线 42.6%**；② 评测对齐后成为可信标尺，后续优化（攻检索 A 类 / 生成覆盖 B 类）都在真实基线上度量；③ 方向增强未在评测复现（症状运行时才有），真·生产实际值略高于 42.6%，是保守下限；④ 遗留真实检索缺口（已被本次对齐证实命中的）：剂量查询类整档、荨麻疹急诊指征(0.0)、呕吐伴剧痛鉴别(0.17)、布洛芬每日最大剂量(0.25)等，下步主攻。

<footer>v9.70 · app/rag/evaluation.py、app/graph/nodes/nodes.py、CHANGELOG.md</footer>

## v9.69b - 生成覆盖prompt实验（负结果，已回滚）：并列/表格强制全列 48.1%→40.7%（app/graph/nodes/prompts.py）

背景：v9.69 检索召回 75.9% vs 最终 avg_hit 0.678，未命中要点里 **B 类（检索已召回、答案没写全）占 57%（52 个）**，且大量样本 hit 卡 0.5–0.8、往往只差 1–2 个要点没过 0.7。以为可借"有针对性提示"吃掉这段损耗。

- **改动（两条温和提示，未碰忠实度根本）**：① 正文 bullet 增补"文档以并列条目/列表/对比表格记载的多条同类信息要逐一给出（多档剂量/多行对比差异），不因'要全'而凭常识补写"；② "需立即就医"节增补"文档列出的每一个危险信号/红旗征都值得转述"。
- **验证（`my_medical_env`，`ENABLE_SEMANTIC_CACHE=false`，温度0 同口径）**：通过率 **26/54=48.1% → 22/54=40.7%**，avg_hit 0.678→0.627，**明确反噬**。与 v9.67 曾回滚的"prompt 覆盖强化尝试"结论一致——**这个评测里任何"要覆盖全"的生成指令都会降通过率**（鼓励多枚举会稀释重点、改变措辞与条目结构，被 LLM 语义判定判为失配），已完整回滚，`git diff app/graph/nodes/prompts.py` 为空。
- 教训固化：黄金评测的 B 类损耗**不能用给生成模型施压"写更全"来解决**，这条路证明是死胡同（两次独立实验同结论）。剩余改进只能来自检索侧的候选覆盖（A 类 39 个），或接受部分"黄金 key_facts 与文档取值/措辞矛盾"（如鼻出血答案"5-10分钟" vs 金标"10-15分钟"、偏头痛文档两节同药剂量 200-400 与 400-600）是评测标尺自身问题、模型怎么改都对不了的硬底。

<footer>v9.69b · app/graph/nodes/prompts.py（已还原）、CHANGELOG.md</footer>

## v9.69 - 检索精度提升二连：医疗同义词表扩容 + 邻域扩展/截断上限拉大（app/rag/entity_overlap.py、app/core/config.py、app/graph/nodes/nodes.py）

背景：v9.68 用 LLM 语义判定跑黄金集全量，拿到可信的**检索层要点召回率 70.9%（200/282）**，发现：① 最终答案命中率（~0.66）≈ 检索召回（0.71）只折损 ~5pp——**生成层几乎无损，主战场是召回**，通过率被检索召回 70.9% 这个硬上限卡死（理论最好通过率仅 63%）；② 失败要点里"数值/细则"和"多要点聚合"两类最多，急诊急救（拨打120/狗咬伤需跨多章节）上下文被截断、《比对/禁忌表》关键行召回不全。

- **P0-1 医疗同义词表扩容（entity_overlap.py `SYMPTOM_SYNONYMS`）**：新增实测缺口对——`狗咬伤/狗咬→犬咬伤/犬咬`、`退烧药/退热药/退烧/退热→解热镇痛`、`吐血→呕血`、`打120→拨打120`。方向对准文档用词（犬咬伤/解热镇痛/呕血/拨打120 均在 docs 存在校验通过）。经 `expand_term` 双向等价类，`has_query_overlap`（过滤层）与 `_entity_anchored_search`（锚定检索 normalize_term）同时受益。
- **P0-2 邻域扩展与文档截断上限拉大（config.py + nodes.py）**：`SIBLING_WINDOW` 1→2、`MAX_SIBLING_CHARS` 1500→2800，修"多要点聚合型"查询（拨打120/高血压日常注意/狗咬伤流程等需跨 2+ 章节）被兄弟扩展 1500 字符上限截断、共用要点召不齐；`build_rag_prompt` 单文档截断 2000→3000，避免父文档/兄弟章节被单文档截断而再次漏掉共用要点。取舍：上下文预算小幅上升、TTFT 略增。
- 检索测试要点：**测试与 v9.65~/9.66 一致，关语义缓存（`ENABLE_SEMANTIC_CACHE=false`）跑全量新鲜检索**，避免 L2 缓存命中旧检索结果掩盖检索侧改动（P0-2 发生在检索器内部，缓存命中会完全测不出）。

验证（`my_medical_env`，`ENABLE_SEMANTIC_CACHE=false`）：
- **检索层要点召回率 70.9% → 75.9%**（+5.0pp，+14 要点）；每条全命中样本 14→18；A 类（0 召回段）样本 1→0。
- **关键样本修复**：被狗咬伤 0/6→6/6、布洛芬和对乙酰氨基酚可同吃 0/4→4/4、孕妇吃布洛芬 2/3→3/3、布洛芬禁忌 4/5→5/5、对乙酰氨基酚和布洛芬哪个好 4/6→6/6、腰椎间盘突出 4/5→5/5、颈椎病分型 6/6、呕吐伴头痛 6/6；拨打120 1/7→3/7（仍低）。
- **黄金评测（关缓存、温度0 确定性标尺）**：通过率 25/54=46.3%（v9.67 同条件基线）→ 26/54=48.2%，avg_hit 0.664→0.678；理论上限通过率（检索召回≥0.7 的样本占比）63%→70%。
- 残留低召回（下一步目标）：6个月宝宝剂量(0/5)、高血压日常注意(0/6)、胃食管反流(1/5)、感冒抗生素(2/5)、复方感冒药(2/6)、便秘(2/5)、呕血黑便(2/5)。

诚实结论：检索召回 +5.0pp 是直接可归因的硬提升；但黄金通过率只 +1.9pp（46.3→48.2），因为检索召回的增量被"生成层再掉 ~7pp"（召回 75.9% → 最终 avg 0.678）和"部分样本差临门一脚没超 0.7 阈值"双重稀释。检索 70% 的通过上限已抬到，下一步若想继续冲通过率，应转向**生成层对已召回要点的完整覆盖**（B 类）与剩余低召回样本的候选排序（C 类），而非继续加深检索量。另存疑：狗咬伤曾由 v9.65 修到 6/6，本次诊断前回退到 0/6——疑似后续集合切换/候选排序回退，本次 P0-2 已救回，但根因（单短 chunk 候选排名易输给泛"处理"词文档）未根治，另案跟踪。

<footer>v9.69 · app/rag/entity_overlap.py、app/core/config.py、app/graph/nodes/nodes.py、CHANGELOG.md</footer>

## v9.68 - 召回诊断升级为 LLM 语义判定，输出可信的检索层要点召回率（scripts/diag_recall.py）

背景：v9.67 的 `diag_recall.py` 用**纯字面子串匹配**判断"key_fact 是否在召回 chunk 里"，据此把失败样本归 A 类（检索缺失）/ B 类（生成覆盖不全）。但 key_facts 措辞常与召回片段错开（换同义词/语序/更具体/引用），字面判"缺失"大量是假阴性——与黄金评测曾踩的坑同源（见记忆 feedback_golden_eval_semantic，字面 5.5% vs 语义 45.5%）——**A/B 分类不可信，检索层真实召回率测不出来**。

- **两级判定（先字面定位、未命中交 LLM 语义复审）**：`_judge_sample` 先对每条 key_fact 做归一化字面定位并回显命中段落号；字面未定位的要点批量交 `_llm_judge_recall`（一次调用判一批，判据与黄金评测一致：换说法/更具体/同义都算到达）。LLM 判"缺失"的新标记为 `[缺失]`，被判"到达"的可信标记为 `[命中·LLM语义]` + 理由。
- **可信的检索层要点召回率汇总**：报告末尾输出 总体命中要点/总要点 = 召回率（按 key_fact 加权）、平均每条未加权召回率、全命中样本数、A 类候选（0 召回段）样本数。诊断固定 `MODEL_TEMPERATURE=0.0` 保证可复现（沿用 v9.67 评测标尺做法，仅本进程）。
- **保留 `--no-semantic`** 可退回纯字面模式做新旧对比。
- 冒烟验证（`my_medical_env`，`ENABLE_SEMANTIC_CACHE=false`）：「布洛芬的禁忌症」4/5（缺失"不可与阿司匹林同服"）、「布洛芬每天最大剂量」3/4（缺失"24小时不超过1200mg"）——这些"缺失"是 LLM 确认召回上下文真没到，属真 A 类候选，旧版字面却会把 `[命中]` 的段落误报或把真缺失掩盖。

后续：拿这份可信召回率跑黄金集全量，可把失败样本精确归 A（检索缺失）/ B（召回在但生成没答全），再决定打检索深度还是生成覆盖。全量跑较慢（每条约检索 1-2s + LLM 批量判定），按需执行。

<footer>v9.68 · scripts/diag_recall.py、CHANGELOG.md</footer>
...
- **体温多档分阶**：`_normalize_vital_temperature` 由"发热/体温正常"两态改为四档——`<37.3 体温正常`、`[37.3, 38) 低热`、`[38, 39) 中热`、`>=39 高热`（阈值常量 `_FEVER_TEMP_THRESHOLD/_LOW_FEVER_TEMP_MAX/_MID_FEVER_TEMP_MAX` 可配）。返回的档位词与规则表低/中/热映射对齐，数值体温如今能触发对应方向规则。
- **规则词向 keyword_matcher 标准词对齐并扩充**：① 新增 低热/中热/高热 三档独立规则（`低热→上呼吸道感染/低热`、`中热→感染/上呼吸道感染`、`高热→呼吸道感染/感染`）；② alias 归一化反向对齐标准词——`咽痛/咽喉痛/喉咙痛→嗓子疼`、`发烧→发热`、补 `腹疼→腹痛`，修掉 v9.51 漏掉"嗓子疼→咽痛"的关键 bug；③ 扩充组合规则至 30 条（叠加 咳嗽+呼吸困难→肺炎、咳嗽+胸痛→肺部感染、腹痛+呕吐/腹泻/恶心→急性肠胃炎/胃炎、胸痛+呼吸困难/胸闷+呼吸困难→心血管疾病、皮疹+瘙痒→过敏性皮炎、水肿→肾脏疾病、头晕+呕吐→眩晕、便秘→便秘/消化功能紊乱 等），关键词全部取 keyword_matcher 确能产出的标准词，降低 LLM 兜底率、提升检索针对性。
- **判合规整**：规则判合改为"精确相等 或 规则词⊆症状词"的正向匹配，移除原反向 `s in kw`，修复"短症状词恰为规则词子串"导致的伪命中；单字规则词不参与子串匹配（`len(kw)>=2`）。
- **确定性单测**：`tests/test_nodes.py` 新增 `TestDiseaseDirection` 共 19 条——温度四档边界（37.3/38/39 含与不含）、A 路径规则命中且 **断言 LLM 兜底不被调用**（`patch _infer_disease_direction_llm` 校验"嗓子疼/喉咙痛/又吐又拉/胸闷呼吸困难"不再烧 LLM）、情绪 aliasing 归一化、反向子串不再伪命中、B 路径 LLM 兜底触发、空 query/空症状原样返回、`_enrich_disease_direction` 拼合/去重/超长兜底/空输入不变量。
- 验证：新增 `TestDiseaseDirection` 19/19 通过；全量 `tests/test_nodes.py` 列表 **16 失败为基线既有问题**（stash 校验：改动前同样 16 失败、症状白名单清洗/关文档用药剥离/路由/查询相等性，与 v9.51 时记录的基线一致，本次改动零回归）。
- 说明：规则判合仅作用于方向增强，未改 `state.py` schema，方向词仍只经 `_enrich_disease_direction` 进检索用 search_query，绝不触碰 `rewritten_query`/`final_question`，医疗诊断断言风险保持规避。

<footer>病症方向检索增强四项优化 · 改动文件：`app/graph/nodes/nodes.py`、`tests/test_nodes.py`、`CHANGELOG.md`</footer>
...
- **根因**：`_decode_answerable` 步骤4 `_context_has_entity(history_summary, state)` 把**所有来源的累积实体**（含 checkpouncer 之外更早轮次的 user_profile.confirmed_facts/allergies）都当"可补出实体"授权检索。但对"吃了三粒"这类**新鲜摄入事件报告**，历史里别的药与当前"是哪一种药"毫无关系——历史不授权。这是"自动驾驶式"漏洞：真实 user 测过别的线索后，"三粒"这个动作会被旧实体豁免。
- **修复（事件优先于档案授权）**：新增 `_is_drug_consumption_fragment()`，判据＝**摄药动词（吃/服/吞/误食/误服/误吞）+ 剂型量词（粒/片/颗/包/丸/袋/毫克/克）**同时在场即视为"摄入事件报告"。该判据**置于 `_context_has_entity` 之前**：一旦判定为摄入碎片（即使历史/档案累积了旧药也强制澄清，历史无从判断是哪种药）。并豁免"自带药物主语"的 via 情况：问句若含 `_has_specific_health_topic`（如"血压药吃了三粒"已点名药物）则不算碎片、照常放行，避免误杀已点名药的剂量咨询。
- 与 v9.46 量词碎片分支（步骤6）的区别：步骤6只认"短查询+疑问词开头+无实体"，会漏掉"服用二片会怎么样/误服一颗咋办/三片吃了会出问题吗"这类**嵌入完整句式、无疑问词开头**的摄入报告；v9.48 的动词+量词判据把这些变体一网打尽。
- 验证：5 类摄入事件变体（吃了三粒怎么办/服用二片会怎么样/误服一颗咋办/三片吃了会出问题吗/误食一包有什么影响）→ 全部 `clarify`；"血压药吃了三粒"→`ok`（点名药）；"一天几次？"带药历史 →`ok`（合法剂量追问不误伤）；"二甲双胍一天几次"→`ok`。澄清测试类 16/16 通过。

<footer>摄入事件强制澄清 · 改动文件：`app/graph/nodes/nodes.py`、`tests/test_nodes.py`、`CHANGELOG.md`</footer>
