...
- 只输出自然语言回答，严禁输出任何 JSON/结构化数据或代码块，严禁复述或附带【L2 临床快照】等结构化键值数据"""),
    ("human", """【文档】
{context}
...
- 逐条转述文档中记载的处理方法：用药（名称与文档一致，剂量按原文）、物理/家庭护理措施、生活调整。你的"自由发挥"只能体现在**组织与措辞**上，不能改变事实：每一条护理/用药措施都必须能在【文档】中找到原文，不得凭常识或自己的知识补充（如"增加衣物保暖"这类文档相反/未记载的措施），文档没写的数字/时长/步骤一律不得编造
- 某类措施（用药/物理护理/生活调整）文档未记载时，该条不写即可，无需刻意提示"文档未提供"
- 若问题涉及如何处理/缓解/用药，必须以"怎么做"为核心逐条给出，不要只罗列现象

**多症状逐一作答（重要）**：若【问题】涉及多个症状/多个不适诉求（如"我前几天肚子不舒服，今天有点发烧"），必须对每个症状分别给出处理建议，各成一条 bullet（用"- 症状名：…"开头），不得合并或只挑其一：
- 某症状在【文档】中找不到对应处理时，在该症状下明确写"文档未提供该症状的具体处理建议"，再举例如为依据不足建议就医；绝不因文档缺该症状而跳过、遗漏或擅自凭常识补写。
- 判断依据是【问题】文本本身提到的症状，而非【文档】中检索到的内容；宁可多列一个症状，也不要漏答用户明确提出的症状。
- **只对【问题】里这次明确提到的症状逐一作答**。若本次只提了一个症状（如"发烧了怎么办"），就只答该症状；【L2 临床快照】【L3 对话历史】里记录的其他症状（如既往的头痛、腹痛）只是既往背景，不是你本次要处理的多个诉求，**不得把它们也当作"多症状"逐条列处理建议**。背景仅供判断用药、禁忌与相互关注，是否顺带提及由你按必要判断，但不得伪装成用户本次的诉求。
- 多症状部分必须是 bullet 形式（"- 症状：…"），便于系统按点核对。

**需立即就医**（可选小节）
- **仅当**文档明确提到红旗征/危险信号/需就医情况时，才用一个小节逐条列出（标题可自定，如"需立即就医"）；文档未提及则**不要**写这一节。

**当前资料未提及的关键方面**（可选小节）
- **仅当**文档确实缺少对回答至关重要的信息（如禁忌症、用法用量、适用人群）时，用一两句话简要说明；信息充分则**省略**此节，不要为凑结构硬写。
